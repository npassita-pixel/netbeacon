...
/* ─── AXE-CORE RUNNER (runs axe on HTML via hidden iframe) ──── */
async function runAxeOnHtml(html) {
  return new Promise((resolve) => {
    try {
      const iframe = document.createElement("iframe");
      iframe.style.cssText = "position:absolute;left:-9999px;width:1280px;height:800px;border:none;visibility:hidden";
...
function mapAxeResults(axeResults) {
  if (!axeResults) return { issues: [], passes: [], ded: 0 };
  const issues = [], passes = [];
  let ded = 0;

  function axeWcag(tags) {
    for (let i = 0; i < tags.length; i++) {
      const m = tags[i].match(/^wcag(\d)(\d)(\d+)$/);
  ...
  axeResults.violations.forEach(v => {
    ...
    if (/^<img /i.test(snippet)) {
      const sm = snippet.match(/src=["']([^"']+)["']/i);
      if (sm) imgSrc = sm[1];
    }
    const instances = [];
    if (v.nodes.length > 1) {
      v.nodes.slice(0, 20).forEach(n => {
        const h = (n.html || "").slice(0, 200);
        let iS = "";
        if (/^<img /i.test(h)) {
          const ns = h.match(/src=["']([^"']+)["']/i);
          if (ns) { iS = ns[1]; imgSrcs.push(ns[1]); }
        }
    ...
    issues.push({
      severity: sev,
      title: v.help + (v.nodes.length > 1 ? ` (${v.nodes.length})` : ""),
      wcag, principle,
...
<div class="footer">
  <div>Generated by Beacon Scanner — ${date} — Automated scan covering WCAG 2.1 AA, WCAG 2.2, ADA Title III &amp; Section 508. Manual testing recommended for full compliance.<br>Core accessibility checks powered by axe-core® by Deque Systems</div>
</div>
</div>
</body></html>`;
  ...
  const scan = async () => {
    ...
    try {
      ...
      if (inputMode === "paste") {
        ...
        // Run axe-core on pasted HTML via hidden iframe
        setProgress("Running axe-core checks…"); setProgressPct(25);
        const axeResults = await runAxeOnHtml(html);
        const axeMapped = mapAxeResults(axeResults);

        // Run local WCAG scan for custom checks
        setProgress("Running custom checks…"); setProgressPct(30);
        const localResult = runLocalScan(parsedDoc, baseUrl);

        // Merge: axe results + local custom checks (dedup by wcag criterion, prefer axe)
        const axeWcagSet = new Set(axeMapped.issues.map(i => i.wcag));
        const customOnly = localResult.issues.filter(i => !axeWcagSet.has(i.wcag));
        const mergedIssues = [...axeMapped.issues, ...customOnly];
        const axePassWcag = new Set(axeMapped.passes.map(p => p.wcag));
        const customPasses = localResult.passes.filter(p => !axePassWcag.has(p.wcag));
        const mergedPasses = [...axeMapped.passes, ...customPasses];

        // Recalculate score with merged results
        const mergedDed = axeMapped.ded + customOnly.reduce((d, i) => {
          if (i.severity === "critical") return d + 7;
          if (i.severity === "serious") return d + 5;
          if (i.severity === "moderate") return d + 3;
          return d + 1;
        }, 0);
        const critC = mergedIssues.filter(i => i.severity === "critical").length;
        ...
        try {
          ...
          if (html) {
            ...
            // Run axe-core on proxy HTML via hidden iframe
            setProgress("Running axe-core checks…"); setProgressPct(28);
            const axeRes = await runAxeOnHtml(html);
            const axeM = mapAxeResults(axeRes);

            // Run local custom checks
            setProgress("Running custom checks…"); setProgressPct(30);
            const localResult = runLocalScan(parsedDoc, baseUrl);

            // Merge: axe + custom (dedup by wcag, prefer axe)
            const axeW = new Set(axeM.issues.map(i => i.wcag));
            const custOnly = localResult.issues.filter(i => !axeW.has(i.wcag));
            const mIssues = [...axeM.issues, ...custOnly];
            const axePW = new Set(axeM.passes.map(p => p.wcag));
            const mPasses = [...axeM.passes, ...localResult.passes.filter(p => !axePW.has(p.wcag))];
            const mDed = axeM.ded + custOnly.reduce((d, i) => d + (i.severity === "critical" ? 7 : i.severity === "serious" ? 5 : i.severity === "moderate" ? 3 : 1), 0);
            const cC = mIssues.filter(i => i.severity === "critical").length;
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, fontFamily:"'DM Sans',sans-serif", color:C.text }}>
      ...
      <main id="main-content" style={{ maxWidth:960, margin:"0 auto", padding:"28px 28px 60px" }}>
        ...
        {result && !loading && (
          <p style={{ textAlign:"center", fontSize:11, color:C.textMuted, margin:"24px 0 0", padding:"12px 0 0", borderTop:`1px solid ${C.border}` }}>
            Core accessibility checks powered by axe-core® by Deque Systems
          </p>
        )}
      </main>
    </div>
